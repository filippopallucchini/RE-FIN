...
def main():
    ...
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if f'file_{num_candidates}_candidates_corrected_for_enrichment_fiqa.pkl' == filename:
    ...
    if check_presence_candidates_corrected == False:   
        ...
        #save possible enriched sentences
        # create a binary pickle file 
        f = open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fiqa.pkl","wb")
        # write the python object (dict) to pickle file
        pickle.dump(list_input_enrichment_top_candidates_corrected,f)
        # close file
        f.close()
    else:
        list_input_enrichment_top_candidates_corrected = pickle.load(open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fiqa.pkl", 'rb'))
    ...
    output_fiqa_enriched.to_csv(f'{path}/OUTPUT/fiqa_enriched.csv')
