...
def select_top_enrichment_candidates(input_sentence, candidates, model_encoder_only, num_top_candidate):
    ...
    input_embedding = model_encoder_only.encode(input_sentence)
    #find best candidate
    #cos_scores = util.pytorch_cos_sim(rotated_vector, enriched_sentences_embedding)[0]
    cos_scores = util.pytorch_cos_sim(input_embedding, enriched_sentences_embedding)[0]
...
def select_best_enrichment_candidate(input_sentence, top_candidates_corrected, model_encoder_only, num_top_candidate, limit_fuzzy_enr, limit_semantic_enr):
    enriched = False
    #check similarities of new and old sentences
    ...
    for enriched_sentence in top_candidates_corrected_sort:
        ...
        if len(enriched_sentence) < len(input_sentence) and rapidfuzz.fuzz.token_set_ratio(input_sentence_clean, enriched_sentence_clean) > limit_fuzzy_enr:
            ...
            #print(f'sentence_for_enrichment: \n{sentence_for_enrichment}')
            #print(f'enriched_sentence:\n{enriched_sentence}')
            break
        else:
            #print('no: too much similar')
            enriched_sentence = input_sentence
...
def main():
    ...
    with open(f'{path}/FPB/DATA/file_subsentences_densex_fpb.pkl', 'rb') as f:
    ...
    gold_standard = []
    predictions = []
    gold_standard_enrichment = []
    predictions_enrichment = []
    count_enrichment = 0
    count_no_retrieve = 0
    ...
    for file in os.listdir(directory):
        ...
        if 'file_input_and_sentences_for_enrichment_fpb.pkl' == filename:
    ...
    if check_presence_knowledge_retrieved == False:
        ...
        for index, input in tqdm(enumerate(dataset['train'])):
            ...
            if retrieved_bin == False:
                ...
                f = open(f"{path}/DATA/file_input_and_sentences_for_enrichment_fpb.pkl","wb")
                ...
                # create a binary pickle file 
                f = open(f"{path}/DATA/file_input_and_sentences_for_enrichment_true_fpb.pkl","wb")
                # write the python object (dict) to pickle file
                pickle.dump(list_input_and_sentences_for_enrichment_true,f)
                # close file
                f.close()
    else:
        list_input_and_sentences_for_enrichment_true = pickle.load(open(f'{path}/DATA/file_input_and_sentences_for_enrichment_true_fpb.pkl', 'rb'))
    ...
    for input_sentence, sentence_for_enrichment, candidates in tqdm(list_input_and_sentences_candidates):
        ...
        top_candidates = select_top_enrichment_candidates(input_sentence, candidates, model_encoder_only, num_top_candidate)
    ...
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if f'file_{num_candidates}_candidates_corrected_for_enrichment_fpb_ablation_no_mt.pkl' == filename:
    ...
    if check_presence_candidates_corrected == False:   
        ...
        for input_sentence, sentence_for_enrichment, candidates, top_candidates in tqdm(list_input_and_sentences_and_top_candidates):
            if len(input_sentence) > 314:
                list_input_enrichment_top_candidates_corrected.append((input_sentence, sentence_for_enrichment, candidates, top_candidates, ['']))
            else:
        ...
        #save possible enriched sentences
        # create a binary pickle file 
        f = open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fpb_ablation_no_mt.pkl","wb")
        # write the python object (dict) to pickle file
        pickle.dump(list_input_enrichment_top_candidates_corrected,f)
        # close file
        f.close()
    else:
        list_input_enrichment_top_candidates_corrected = pickle.load(open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fpb_ablation_no_mt.pkl", 'rb'))
    ...
    for input_sentence, sentence_for_enrichment, candidates, top_candidates, top_candidates_corrected in tqdm(list_input_enrichment_top_candidates_corrected):
        if len(top_candidates_corrected) == 0:
            ...
            enriched, sentence_enriched = select_best_enrichment_candidate(input_sentence, top_candidates_corrected, model_encoder_only, num_top_candidate, limit_fuzzy_enr, limit_semantic_enr)
    ...
    output_fpb_enriched.to_csv(f'{path}/OUTPUT/fpb_enriched_allagree_ablation_no_mt.csv')
