...
def main():
    ...
    dataset = load_dataset("financial_phrasebank", "50agree")
    ...
    data_sentences_enriched = {'input_sentence': dataset['train']['sentence'],
            'element_for_enrichment': list_elements_of_enrichment
            }

    # Create DataFrame
    output_sentences_enriched = pd.DataFrame(data_sentences_enriched)
    output_sentences_enriched.to_csv(f'{path}/DATA/check_subsentences_densex_fpb.csv')

    # create a binary pickle file 
    f = open(f"{path}/DATA/file_subsentences_densex_fpb.pkl","wb")
