...
def select_top_enrichment_candidates(input_sentence, sentence_for_enrichment, candidates, model_encoder_only, proximity_to_retrieved, num_top_candidate):
    ...
    #midpoint = input_embedding + (sentence_for_enrichment_embedding - input_embedding)/5
    #angle_degrees = angle(input_embedding, sentence_for_enrichment_embedding)
    #rotated_vector = rotate_vector(input_embedding, angle_degrees)
...
def main():
    ...
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if f'file_{num_candidates}_candidates_corrected_for_enrichment_fpb.pkl' == filename:
    ...
    if check_presence_candidates_corrected == False:   
        ...
        #save possible enriched sentences
        # create a binary pickle file 
        f = open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fpb.pkl","wb")
        # write the python object (dict) to pickle file
        pickle.dump(list_input_enrichment_top_candidates_corrected,f)
        # close file
        f.close()
    else:
        list_input_enrichment_top_candidates_corrected = pickle.load(open(f"{path}/DATA/file_{num_candidates}_candidates_corrected_for_enrichment_fpb.pkl", 'rb'))
    ...
    output_fpb_enriched.to_csv(f'{path}/OUTPUT/fpb_enriched_allagree.csv')
